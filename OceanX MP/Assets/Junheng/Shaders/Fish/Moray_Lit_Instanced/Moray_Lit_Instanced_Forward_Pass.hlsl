#ifndef MORAY_UNIVERSAL_FORWARD_LIT_PASS_INCLUDED
#define MORAY_UNIVERSAL_FORWARD_LIT_PASS_INCLUDED

// Moray forward pass. Identical to Fish_Lit_Instanced_Forward_Pass EXCEPT the vertex placement:
// instead of rotating a rigid mesh by LookRotation(direction), it sweeps each vertex along the
// moray's recorded head-path (ApplyMoraySpine). Everything downstream (lighting, fog, GI) is stock.

#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Lighting.hlsl"
#if defined(LOD_FADE_CROSSFADE)
    #include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/LODCrossFade.hlsl"
#endif
// Utils gives LookRotation / CreateRotationMatrix (debug-straight A/B path) + GetVertexNormalInputs.
...
Varyings LitPassVertex(Attributes input, uint svInstanceID: SV_InstanceID)
{
    ...
    // The moray slice starts at _BoidsBufferOffset, so the instance index within the slice IS the
    // moray-local index that keys the trail buffers.
    ...
    {
        // A/B baseline: the old rigid placement (LookRotation about the mesh centre, no swim wobble).
    ...
    // Optionally flip normals+tangent for a mirrored / reverse-wound FBX import (the moray model appears to
    // be one — it also needed Cull Off). Without this the lit surface faces inward and reads as dark /
    // "absorbing the scene" once both sides are drawn.
    ...
    // normalWS and tangentWS already normalize. Uses the per-vertex spine frame, so normals bend
    // with the body along the curve.
