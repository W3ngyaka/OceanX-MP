...
Varyings LitGBufferPassVertex(Attributes input)
{
    ...
    float3 originalPosition = input.positionOS.xyz;
    // Applying fish swimming motion to the vertex position.
    float3 displacedPosition = ApplyRayWingMotion(originalPosition, input.tailMaskUV);
...
FragmentOutput LitGBufferPassFragment(Varyings input)
{
    ...
    if(_VisualizeMask > 0.0) {
        float tailMask = pow(saturate(input.uv.z), max(_WingAmpFalloff, 1.0));
