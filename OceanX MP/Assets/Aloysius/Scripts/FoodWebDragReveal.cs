...
public class FoodWebDragReveal : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    void Start()
    {
        if (currentOrganismsView != null)
        {
            ...
            if (cg == null) cg = currentOrganismsView.gameObject.AddComponent<CanvasGroup>();
    ...
    public void OnPointerUp(PointerEventData e)
    {
        ...
        float dragDistance = pointerDownPos.y - e.position.y;
...
public void OpenView()
    {
        if (currentOrganismsView == null) return;
        ...
        if (dimOverlay != null)
        {
            ...
            StartCoroutine(FadeCanvasGroup(dimOverlay, 0f, 1f, animDuration));
        }

        StartCoroutine(Animate(hiddenPos, shownPos, 0f, 1f));
    }
...
public void CloseView()
    {
        ...
        {
            dimOverlay.blocksRaycasts = false;
            StartCoroutine(FadeCanvasGroup(dimOverlay, 1f, 0f, animDuration, () =>
            {
                dimOverlay.gameObject.SetActive(false);
            }));
        }

        StartCoroutine(Animate(shownPos, hiddenPos, 1f, 0f, () =>
        {
            currentOrganismsView.gameObject.SetActive(false);
        }));
    }

    IEnumerator Animate(Vector2 fromPos, Vector2 toPos, float fromAlpha, float toAlpha, System.Action onComplete = null)
    {
        cg.interactable = false;
        cg.blocksRaycasts = false;
        ...
        while (t < 1f)
        {
            ...
            currentOrganismsView.anchoredPosition = Vector2.Lerp(fromPos, toPos, eased);
    ...
    IEnumerator FadeCanvasGroup(CanvasGroup target, float from, float to, float duration, System.Action onComplete = null)
    {
        float t = 0f;
        while (t < 1f)
        {
