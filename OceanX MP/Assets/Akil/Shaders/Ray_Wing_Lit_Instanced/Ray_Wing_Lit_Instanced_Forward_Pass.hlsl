...
Varyings LitPassVertex(Attributes input, uint svInstanceID: SV_InstanceID)
{
    ...
    // Apply wing-flap motion to the original vertex position. The ray motion is driven by
    // the material properties (and _Time), so it does not need any per-boid swim parameters.
